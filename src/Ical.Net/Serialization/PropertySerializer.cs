...
namespace Ical.Net.Serialization
{
    public class PropertySerializer : SerializerBase
    {
        ...
        public override string? SerializeToString(object? obj)
        {
            ...
            SerializeToString_(prop, sf, result);
        ...
        void SerializeToString_(ICalendarProperty prop, ISerializerFactory sf, StringBuilder result)
        {
            foreach (var v in prop.Values.Where(value => value != null))
            {
                SerializeToString_(prop, sf, result, v);
            }
        }

        void SerializeToString_(ICalendarProperty prop, ISerializerFactory sf, StringBuilder result, object v)
        {
            ...
            {
                if (!string.IsNullOrWhiteSpace(typed.TzId) && parameterList.All(p =>
                        string.Equals("TZID", p.Value, StringComparison.OrdinalIgnoreCase)))
                {
            ...
            if (parameterList.Any())
            {
                ...
                if (sf.Build(typeof(CalendarParameter), SerializationContext) is IStringSerializer parameterSerializer)
                {
                    ...
                    sb.Append(string.Join(";",
                        parameterList.Select(param => parameterSerializer.SerializeToString(param))));
                }
            }
            sb.Append(":");
            sb.Append(value);
