...
public class FreeBusyEntry : Period
{
    ...
    {
        return dt is not null && Contains(dt.AsZonedOrDefault(DateTimeZone.Utc).ToInstant());
    }
    ...
    {
        var startInstant = StartTime.AsZonedOrDefault(DateTimeZone.Utc).ToInstant();
        ...
        {
            end = endTime.AsZonedOrDefault(DateTimeZone.Utc).ToInstant();
        }
    ...
    public bool CollidesWith(Period? period)
    {
        ...
        var start = StartTime.AsZonedOrDefault(DateTimeZone.Utc).ToInstant();
        Instant end;
        if (EndTime is { } endTime)
        {
            end = endTime.AsZonedOrDefault(DateTimeZone.Utc).ToInstant();
        ...
        var otherStart = period.StartTime.AsZonedOrDefault(DateTimeZone.Utc).ToInstant();
        Instant otherEnd;
        if (period.EndTime is { } periodEndTime)
        {
            ...
            otherEnd = periodEndTime.AsZonedOrDefault(DateTimeZone.Utc).ToInstant();
        }
