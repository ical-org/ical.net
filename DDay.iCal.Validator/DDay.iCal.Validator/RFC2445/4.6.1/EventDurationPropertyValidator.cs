...
namespace DDay.iCal.Validator.RFC2445
{
        ...
        IValidator
	{
        ...
        public EventDurationPropertyValidator(iCalendar cal)
        {
            iCalendar = cal;
        }
        ...
        {
            ValidationResult result = new ValidationResult("eventDurationProperty");
            result.Passed = true;
            ...
            foreach (Event evt in iCalendar.Events)
            {
                ValidationResult evtResult = ValidationResult.GetCompositeResults(
                    "eventDurationProperty",
                    new PropertyCountValidator(evt, "VEVENT", "DURATION")                    
                );
