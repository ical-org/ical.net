...
namespace DDay.iCal.Validator.RFC2445
{
        ...
        IValidator
	{
        ...
        public EventTranspPropertyValidator(iCalendar cal)
        {
            iCalendar = cal;
        }
        ...
        {
            ValidationResult result = new ValidationResult("eventTranspProperty");
            result.Passed = true;
            ...
            foreach (Event evt in iCalendar.Events)
            {
                ValidationResult evtResult = ValidationResult.GetCompositeResults(
                    "eventTranspProperty",
                    new PropertyCountValidator(evt, "VEVENT", "TRANSP")                 
                );
