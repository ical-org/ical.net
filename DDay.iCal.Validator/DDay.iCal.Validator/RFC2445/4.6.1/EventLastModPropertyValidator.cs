...
namespace DDay.iCal.Validator.RFC2445
{
        ...
        IValidator
	{
        ...
        public EventLastModPropertyValidator(iCalendar cal)
        {
            iCalendar = cal;
        }
        ...
        {
            ValidationResult result = new ValidationResult("eventLastModProperty");
            result.Passed = true;
            ...
            foreach (Event evt in iCalendar.Events)
            {
                ValidationResult evtResult = ValidationResult.GetCompositeResults(
                    "eventLastModProperty",
                    new PropertyCountValidator(evt, "VEVENT", "LAST-MODIFIED")                    
                );
